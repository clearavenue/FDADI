...
public class ApiQueriesTest {
	...
	public void getLabelTest() {
		...
		byte[] expectedBytes = expected.getBytes(Charset.forName("ISO-8859-1"));
		try {
			String actual = ApiQueries.getLabel("aspirin");
			byte[] actualBytes = actual.getBytes(Charset.forName("ISO-8859-1"));
			assertArrayEquals(expectedBytes, actualBytes);
		} catch (UnirestException e) {
			e.printStackTrace();
			fail("Unirest exception when testing ApiQueries.getLabel()");
		}
