...
public class DrugInteractions {
	...
	/**
	 * Finds all mentions of each drug's generic and brand name within the drug interactions
	 * entry of each other drug.
	 * 
	 * @param drugs Array of generic names of drugs
	 * @return Hashmap where the keys are each drug that has an interaction with another.
	 * The value of each key is a list of the names of drugs which have an interaction with that key
	 * If a drug has no interactions, it will not exist as a key in the map.
	...
	public static HashMap<String, ArrayList<String>> findInteractions(String... drugs) throws UnirestException{
		...
		for(int currentDrug = 0; currentDrug < interactions.length; currentDrug++){
			...
			for(int otherDrug = 0; otherDrug < interactions.length; otherDrug++){
				if(currentDrug != otherDrug && inter.toLowerCase().contains(drugs[otherDrug].toLowerCase())){
