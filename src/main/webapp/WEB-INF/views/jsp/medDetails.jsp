	...
	<div class="container fdadi-template">
		<div class="row">
			<c:forEach var="med" items="${medList}" varStatus="loop">
				<c:set var="shouldShowSideEffects"
					value="${not empty med.getSideEffects() && showSideEffects}"></c:set>
				<c:set var="shouldShowUsage"
					value="${not empty med.getUsage() && showUsage}"></c:set>
				<c:set var="shouldShowInteractions"
					value="${not empty med.getInteractions() && showInteractions}"></c:set>
				<c:set var="shouldShowIndications"
					value="${not empty med.getIndications() && showIndications}"></c:set>
				<c:set var="shouldShowCounterindications"
					value="${not empty med.getCounterindications() && showCounterindications}"></c:set>


				<div class="panel panel-primary">
					<div class="panel-heading">${med.getBrandName() }(Generic
						name: ${med.getGenericName()})</div>

					<div class="panel-body">
						<c:choose>
							<c:when
								test="${ shouldShowSideEffects || shouldShowUsage || shouldShowInteractions || shouldShowIndications || shouldShowCounterindications}">
